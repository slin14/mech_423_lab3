...
void setup_UART(int int_en) {
    P2SEL0 &= ~(BIT5 + BIT6); // UART ports P2.5 and P2.6 // redundant
    P2SEL1 |=  (BIT5 + BIT6); // UART ports P2.5 and P2.6

    UCA1CTLW0 |= UCSWRST;                   // Put the UART in software reset
    UCA1CTLW0 |= UCSSEL__ACLK;              // Run the UART using ACLK
  //UCA1CTLW0 |= UCSSEL__SMCLK;             // Run the UART using SMCLK
    UCA1MCTLW = UCOS16 + UCBRF0 + 0x4900;   // Baud rate = 9600 from an 8 MHz clock
    UCA1BRW = 52;                           // Baud rate = 9600 from an 8 MHz clock
    UCA1CTLW0 &= ~UCSWRST;                  // release UART for operation

    if (int_en)    UCA1IE |= UCRXIE;        // Enable UART Rx interrupt
}
...
{
    while (!(UCA1IFG & UCTXIFG)); // wait until UART not transmitting
